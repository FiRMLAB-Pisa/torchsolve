"""Learn the regularisation weight by differentiating through the solve.

The solve is differentiated implicitly rather than by unrolling: a gradient
arriving at the solution reaches the right-hand side, and anything the operator
closed over, through one more solve. Memory is therefore flat in the iteration
count, so a solve deep enough to converge costs a backward pass no larger than
a shallow one.
"""
...
def main() -> None:
    ...
    for _ in range(60):
        ...
        estimate = conjugate_gradient(
            lambda vector: vector + weight * (difference.T @ (difference @ vector)),
            measured,
            max_iter=200,
            rtol=1e-12,
    ...
    print(f"weight {weights[0]:.3f} -> {weights[-1]:.3f}, "
          f"loss {losses[0]:.4f} -> {losses[-1]:.4f}")

    # What a sweep would have said, for comparison.
    grid = torch.logspace(-2, 2, 40, dtype=torch.float64)
    swept = []
    for candidate in grid:
        estimate = conjugate_gradient(
            lambda vector: vector + candidate * (difference.T @ (difference @ vector)),
            measured,
            max_iter=200,
            rtol=1e-12,
        ).solution
    ...
    print("wrote figures/differentiable_weight.png")
